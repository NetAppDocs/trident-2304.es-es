...
====
.Administración de clústeres de Kubernetes mediante la NetApp Console
[%collapsible]
====
Con Astra Trident mediante la NetApp Console, puede actualizar a la última versión de Astra Trident, agregar y administrar clases de almacenamiento y conectarlas a entornos de trabajo, y realizar copias de seguridad de volúmenes persistentes mediante el Cloud Backup Service. La consola admite la implementación de Astra Trident mediante el operador Trident , ya sea manualmente o usando Helm.
